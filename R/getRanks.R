...
getRanks <- function(mydata_deltaCT) {
  ...
  names(rank.deltaCT)=c("Sample.Name", "Min.CT","Mid.CT","Max.CT", "CT.Mt_ND1.diff", "CT.RPPH1.diff")
  samplelist=levels(as.factor(mydata_deltaCT$Sample.Name))
  
  for (i in 1:length(samplelist)){
    test=na.omit(subset(mydata_deltaCT, Sample.Name==samplelist[i]))
    ...
    if (nrow(test)==3){
      first= which(test.ordered$deltaCT==min(test.ordered$deltaCT), arr.ind=T)
      second= which(test.ordered$deltaCT==median(test.ordered$deltaCT), arr.ind=T)
      third= which(test.ordered$deltaCT==max(test.ordered$deltaCT), arr.ind=T)
      ...
      rank.deltaCT[i,]=c(test$Sample.Name[1], first, second, third, ct.mt.diff, ct.rpph1.diff)
    }
    # this will give you warnings.  the na.omit two lines below this should circumvent that. 
  }
  
  # there are NAs because they don't have 3 samples! so you won't be doing rank-sum analysis on them
